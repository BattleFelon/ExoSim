...
int main(int argc, char** argv)
{
    ...
    float alpha = .02;
    float diff_of_running_and_count = 500;
    ...
    while(is_running)
    {
        ...
        diff_of_running_and_count = (alpha * abs(running_ave - total_count)) + (1.0 - alpha) * diff_of_running_and_count;
        ...
        if(is_stable && is_there){
            if(total_count > running_ave + diff_of_running_and_count * .5){
                is_above = true;
            }
            if(total_count < running_ave - diff_of_running_and_count * .5){
                is_below = true;
